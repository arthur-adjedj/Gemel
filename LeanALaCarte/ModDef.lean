    ...
    withMappedHeadersDecls mapHeaders fun xs => do
      ...
      withAssignableSyntheticOpaque do
        ...
        for {matchName, mvar, originalMatch, modMappedRhss} in matchExtensions, {ref, name, alts, argNames} in matchClauses do
          ...
          withRef ref do
          trace[Modular.Elab] "Elaborating matcher : {name} {Expr.mvar mvar} {(← mvar.isAssigned, ← getExprMVarAssignment? mvar)}"
        ...
        mvars ← mvars.mapM getDelayedMVarRoot
        trace[Modular.Elab] "Mvars post-delay : {mvars.map Expr.mvar}"
        mvars ← mvars.filterM (notM ·.isAssigned)
