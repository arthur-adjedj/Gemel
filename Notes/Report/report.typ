...
$\
"A.rec" => lambda [p_j]_j motive [minor_n]_n space [i_k]_k space a mapsto.long "B.rec"  [p_j]_j motive [minor_n]_n space ?m space [i_k]_k space a$
...
This sort of distinction is not new. Indeed `where finally` was already a feature in Lean, and could be used to fill-in proof holes after the fact:
```lean
